...
//      //  设置Authorization的方法设置header
       ...
       NSString *ScreenId = [[NSUserDefaults standardUserDefaults] objectForKey:@"screen_id"];
     if (access_token.length>0&&ScreenId!=nil) {
          NSString *authorization=[NSString stringWithFormat:@"%@ %@",token_type,access_token];
           ...
           [self.sessionManager.requestSerializer setValue:ScreenId forHTTPHeaderField:@"ScreenId"];
    }
    else
    {
        ...
        case HTTPMethodGET: {
            task = [self.sessionManager GET:requestPath parameters:parameters progress:progress success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
                if (success) {
                    ...
                    if ([dic[@"msg"] isEqualToString:@"token验证未通过"]) {
                        [self goLogin];
                    }
                    else
                    {
...
- (void)errorDispose:(NSError * _Nonnull)error result:(nullable void(^)(BOOL result))resultBlock{
    ...
    if (afNetworking_errorMsg!=nil) {
        serializedData = [NSJSONSerialization JSONObjectWithData: afNetworking_errorMsg options:kNilOptions error:nil];
        NSLog(@"error--%@",serializedData);
    }
...
-(void)goLogin
{
     ...
     [MBProgressHUD showError:@"登录失效，请重新登录" toView:login.view];
