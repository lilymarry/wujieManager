...
- (void)viewDidAppear:(BOOL)animated {
    ...
    [set sUserSettingSuccess:^(NSString *code, NSString *message, id data) {
        ...
        if ([code isEqualToString:@"1"]) {
            ...
            self->_five_iPhone.text = set.data.phone;
            self->iPhone = set.data.phone;
            ...
            if ([set.data.is_password integerValue] == 0) {
                self->_three_Title.text = @"设置登录密码";
                self->three_type = NO;
            } else {
                self->_three_Title.text = @"修改登录密码";
                self->three_type = YES;
            }
            if ([set.data.is_pay_password integerValue] == 0) {
                self->_fourTitle.text = @"设置支付密码";
                self->four_type = NO;
            } else {
                self->_fourTitle.text = @"修改支付密码";
                self->four_type = YES;
            }
            if ([set.data.wx_bind.is_bind integerValue] == 0) {
                self->_bindOneTitle.text = @"未绑定";
                self->_bindOneTitle.textColor = [UIColor redColor];
                self->bindOne_type = NO;
            } else {
                self->_bindOneTitle.text = set.data.wx_bind.bind_info.nickname;
                self->_bindOneTitle.textColor = WordColor_sub;
                self->bindOne_type = YES;
            }
            if ([set.data.qq_bind.is_bind integerValue] == 0) {
                self->_bindTwoTitle.text = @"未绑定";
                self->_bindTwoTitle.textColor = [UIColor redColor];
                self->bindTwo_type = NO;
            } else {
                self->_bindTwoTitle.text = set.data.qq_bind.bind_info.nickname;
                self->_bindTwoTitle.textColor = WordColor_sub;
                self->bindTwo_type = YES;
            }
            if ([set.data.weibo_bind.is_bind integerValue] == 0) {
                self->_bindThreeTitle.text = @"未绑定";
                self->_bindThreeTitle.textColor = [UIColor redColor];
                self->bindThree_type = NO;
            } else {
                self->_bindThreeTitle.text = set.data.weibo_bind.bind_info.nickname;
                self->_bindThreeTitle.textColor = WordColor_sub;
                self->bindThree_type = YES;
            }
            if ([set.data.alipay_bind.is_bind integerValue] == 0) {
                self->_bindFourTitle.text = @"未绑定";
                self->_bindFourTitle.textColor = [UIColor redColor];
                self->bindFour_type = NO;
            } else {
                self->_bindFourTitle.text = set.data.alipay_bind.bind_info.nickname;
                self->_bindFourTitle.textColor = WordColor_sub;
                self->bindFour_type = YES;
            }
    ...
    [alertController addAction:[UIAlertAction actionWithTitle:@"确定" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
        ...
        [self clearCacheResult:^(BOOL result) {
            ...
            if (huancun.doubleValue ==0.00) {
                self->_sexContent.text = @"0MB";
            } else {
                self->_sexContent.text = [NSString stringWithFormat:@"%.2fMB",[self folderFileSizeAtPath]];
            }
        }];
    }]];
    [self presentViewController:alertController animated:YES completion:nil];
